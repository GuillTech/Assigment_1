    ...
    for episode in range(n_episodes):
        ...
        while not done:
            ...
            if len(step_result) == 5:  # New Gym API
                ...
                if isinstance(info, tuple):  # Handle case where info is (info, _)
                    info = info[0]
...
def plot_evaluation_results(
    ...
    mean_reward = float(metrics['avg_reward'])  # Convert to Python float
    ax2.axvline(x=mean_reward, color='r', linestyle='--', label=f"Mean: {mean_reward:.2f}")
...
def compare_models(results: List[Dict], save_path: Path):
    ...
    # Average reward
    ax3 = axes[0, 2]
    avg_rewards = [float(r['metrics']['avg_reward']) for r in results]  # Convert to Python float
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='Eval_result_Wordle',
    ...
    for model_path in model_paths:
        ...
        # Create environment
        env = DummyVecEnv([
            lambda: WordleEnv(use_action_masking=True)
        ])
        ...
        metrics_json = {k: convert_to_python_type(v) for k, v in metrics.items()}
        ...
        print(f"Avg Reward: {float(metrics['avg_reward']):.2f}")
        print(f"Avg Repeated Guesses: {float(metrics['avg_repeated_guesses']):.2f}")
